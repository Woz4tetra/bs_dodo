...
def parse_msg_file(file_path):
    ...
    for line in content.splitlines():
        ...
        if len(parts) >= 1:
            first_word = parts[0].split('[')[0].split('=')[0]  # Get type before array or constant
...
def generate_service_header(spec, root_dir=None):
    ...
    # Calculate MD5 hash for individual request/response (for their own MD5Sum traits)
    request_md5 = generate_md5_hash(spec.request, root_dir)
    response_md5 = generate_md5_hash(spec.response, root_dir)
    ...
    request_text = get_expanded_message_definition(spec.request, root_dir) if root_dir else spec.request.raw_definition
    response_text = get_expanded_message_definition(spec.response, root_dir) if root_dir else spec.response.raw_definition
